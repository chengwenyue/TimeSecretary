...
public class SmackImpl implements Smack {
	...
	public SmackImpl(XXService service) {
		...
		if (customServer.length() > 0
			...
			this.mXMPPConfig = new ConnectionConfiguration("115.159.78.134"); // use SRV
			Log.i(this.getClass().toString(),server);
		}
		...
		try {
			if (mXMPPConnection.isConnected()) {
				try {
					...
					LogUtils.d("conn.disconnect() failed: " + e);
				}
			}
			...
			LogUtils.e(SmackImpl.class+"login(): " + Log.getStackTraceString(e));
	...
	private class PongTimeoutAlarmReceiver extends BroadcastReceiver {
		public void onReceive(Context ctx, Intent i) {
			LogUtils.d("Ping: timeout for " + mPingID);
	...
	private class PingAlarmReceiver extends BroadcastReceiver {
		public void onReceive(Context ctx, Intent i) {
			if (mXMPPConnection.isAuthenticated()) {
				...
				LogUtils.d("Ping: alarm received, but not connected to server.");
		}
	}
	...
	private void registerRosterListener() {
		...
		mRosterListener = new RosterListener() {
			...
			public void presenceChanged(Presence presence) {
				LogUtils.i("presenceChanged(" + presence.getFrom() + "): " + presence);
			...
			public void entriesUpdated(Collection<String> entries) {
				LogUtils.i("entriesUpdated(" + entries + ")");
			...
			public void entriesDeleted(Collection<String> entries) {
				LogUtils.i("entriesDeleted(" + entries + ")");
				for (String entry : entries) {
			...
			public void entriesAdded(Collection<String> entries) {
				LogUtils.i("entriesAdded(" + entries + ")");
	...
	private void addRosterEntryToDB(final RosterEntry entry) {
		...
		LogUtils.i("addRosterEntryToDB: Inserted " + uri);
	}
	...
	private void deleteRosterEntryFromDB(final String jabberID) {
		...
		LogUtils.i("deleteRosterEntryFromDB: Deleted " + count + " entries");
	}
	...
	public void setStatusFromConfig() {
		...
		String statusMessage = PreferenceUtils.getPrefString(mService,
				PreferenceConstants.STATUS_MESSAGE,"在线");
	...
	private void initServiceDiscovery() {
		...
		dm.registerReceiptReceivedListener(new DeliveryReceiptManager.ReceiptReceivedListener() {
			public void onReceiptReceived(String fromJid, String toJid,
					String receiptId) {
				LogUtils.d(SmackImpl.class+ "got delivery receipt for " + receiptId);
	...
	public void removeRosterItem(String user) throws XXException {
		LogUtils.d("removeRosterItem(" + user + ")");
	...
	public void renameRosterGroup(String group, String newGroup) {
		LogUtils.i("oldgroup=" + group + ", newgroup=" + newGroup);
		mRoster = mXMPPConnection.getRoster();
	...
	public void sendServerPing() {
		if (mPingID != null) {
			LogUtils.d("Ping: requested, but still waiting for " + mPingID);
		...
		LogUtils.d("Ping: sending ping " + mPingID);
	...
	public boolean logout() {
		...
		if (mXMPPConnection.isConnected()) {
			...
			new Thread() {
				public void run() {
					LogUtils.d("shutDown thread started");
					mXMPPConnection.disconnect();
					LogUtils.d("shutDown thread finished");
				}
			}.start();
		}
		setStatusOffline();
	...
	public int register(String account, String password, String mail) throws XXException {
		...
		re.setTo(mXMPPConnection.getServiceName());

		re.setPassword(password);
		re.setUsername(account);
		re.addAttribute("email",mail);
		re.addAttribute("android", "geolo_createUser_android");//不能为空

		System.out.println("注册到openfire中的姓名是====="+account+"密码是====="+password);
