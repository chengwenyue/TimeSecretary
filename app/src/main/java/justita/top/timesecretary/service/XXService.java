...
import justita.top.timesecretary.smack.SmackImpl;
import justita.top.timesecretary.uitl.LogUtils;
import justita.top.timesecretary.uitl.NetUtil;
import justita.top.timesecretary.uitl.PreferenceConstants;
import justita.top.timesecretary.uitl.PreferenceUtils;

public class XXService extends BaseService implements TSBroadcastReceiver.EventHandler {
	...
	public IBinder onBind(Intent intent) {
		LogUtils.i(XXService.class+ "[SERVICE] onBind");
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		if (intent != null&& intent.getAction() != null
				...
				TSBroadcastReceiver.BOOT_COMPLETED_ACTION)
				|| TextUtils.equals(intent.getAction(),LOGIN_ACTION))) {
	...
	public void Login(final String account, final String password) {
		...
		if (mConnectingThread != null) {
			LogUtils.i("a connection is still goign on!");
			return;
		}
		mConnectingThread = new Thread() {
			...
			public void run() {
				try {
					...
					LogUtils.i(XXService.class+"YaximXMPPException in doConnect():");
					e.printStackTrace();
				} finally {
					if (mConnectingThread != null)
		...
		if (mConnectingThread != null) {
			synchronized (mConnectingThread) {
				try {
					...
					LogUtils.e("doDisconnect: failed catching connecting thread");
				} finally {
					mConnectingThread = null;
				}
			}
	...
	public void addRosterItem(String user, String alias, String group) {
		try {
			...
			LogUtils.e("exception in addRosterItem(): " + e.getMessage());
		}
	}
	...
	public void removeRosterItem(String user) {
		try {
			...
			Toast.makeText(this,e.getMessage(),Toast.LENGTH_SHORT).show();
			LogUtils.e("exception in removeRosterItem(): " + e.getMessage());
		}
	}
	...
	public void moveRosterItemToGroup(String user, String group) {
		try {
			...
			Toast.makeText(this,e.getMessage(),Toast.LENGTH_SHORT).show();
			LogUtils.e("exception in moveRosterItemToGroup(): " + e.getMessage());
		}
	}
	...
	public void renameRosterItem(String user, String newName) {
		try {
			...
			Toast.makeText(this,e.getMessage(),Toast.LENGTH_SHORT).show();
			LogUtils.e("exception in renameRosterItem(): " + e.getMessage());
		}
	}
	...
	private void connectionFailed(String reason) {
		LogUtils.i(XXService.class+"connectionFailed: " + reason);
		...
		mMainHandler.post(new Runnable() {
			public void run() {
				if (!PreferenceUtils.getPrefBoolean(XXService.this,
					...
					Vibrator vibrator = (Vibrator) XXService.this.getSystemService(Context.VIBRATOR_SERVICE);
					//震动一次
	...
	public void rosterChanged() {
		...
		if (mSmackable != null && !mSmackable.isAuthenticated()) {
			LogUtils.i("rosterChanged(): disconnected without warning");
	...
	Runnable monitorStatus = new Runnable() {
		public void run() {
			try {
				LogUtils.i("monitorStatus is running... " + mPackageName);
				mMainHandler.removeCallbacks(monitorStatus);
				// 如果在后台运行并且连接上了
				if (!isAppOnForeground()) {
					LogUtils.i("app run in background...");
					// if (isAuthenticated())
					updateServiceNotification("后台运行！");
					return;
				} else {
					stopForeground(true);
				}
	...
	private class ReconnectAlarmReceiver extends BroadcastReceiver {
		public void onReceive(Context ctx, Intent i) {
			...
			if (mConnectedState != DISCONNECTED) {
				LogUtils.d("Reconnect attempt aborted: we are connected again!");
				return;
			}
			String account = PreferenceUtils.getPrefString(XXService.this,
					PreferenceConstants.ACCOUNT, "");
